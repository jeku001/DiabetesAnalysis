...
def add_missing_data(df, fraction=0.5):
    """
    Powiela określoną część (fraction) wierszy, dodając braki danych w losowych kolumnach w losowej liczbie od 1 do 2
    """
    ...
    columns_to_consider = df.columns.drop('Outcome')

    for row in duplicated_rows.index:
        num_cols = np.random.choice(range(1, 3))
...
def main():
    df = load_data('medical_diabetes_data.pkl')
